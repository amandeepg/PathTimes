...
class BetterTogether(Teleprompter):
    ...
    def __init__(
        ...
        self.prompt_optimizer = (
            prompt_optimizer
            if prompt_optimizer
            else BootstrapFewShotWithRandomSearch(metric=metric)
        )
        self.weight_optimizer = (
            weight_optimizer if weight_optimizer else BootstrapFinetune(metric=metric)
        )

        is_supported_prompt = isinstance(
            self.prompt_optimizer, BootstrapFewShotWithRandomSearch
        )
    ...
    def _run_strategies(
        self, parsed_strategy, student, trainset, valset_ratio
    ) -> Module:
    ...
    def _compile_prompt_optimizer(self, student, trainset, valset_ratio) -> Module:
        ...
        trainset = [
            x.with_inputs(*list(set(x.inputs().keys()) - {"hint"})) for x in trainset
        ]
        ...
        student = self.prompt_optimizer.compile(
            student, trainset=prompt_trainset, valset=prompt_valset
        )
