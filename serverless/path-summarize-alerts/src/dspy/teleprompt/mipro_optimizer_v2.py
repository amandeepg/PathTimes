...
class MIPROv2(Teleprompter):
    ...
    def compile(
        ...
        *,
        ...
        if self.auto is None and (
            self.num_candidates is not None and num_trials is None
        ):
            raise ValueError(
    ...
    def _set_num_trials_from_num_candidates(
        self, program, zeroshot_opt, num_candidates
    ):
    ...
    def _set_hyperparams_from_run_mode(
        ...
        self.num_instruct_candidates = (
            auto_settings["n"] if zeroshot_opt else int(auto_settings["n"] * 0.5)
        )
        self.num_fewshot_candidates = auto_settings["n"]

        num_trials = self._set_num_trials_from_num_candidates(
            program, zeroshot_opt, auto_settings["n"]
        )
    ...
    def _bootstrap_fewshot_examples(
        ...
        logger.info(
            f"Bootstrapping N={self.num_fewshot_candidates} sets of demonstrations..."
        )
