...
class SIMBA(Teleprompter):
    ...
    def compile(
        self, student: dspy.Module, *, trainset: list[dspy.Example], seed: int = 0
    ) -> dspy.Module:
        """
        ...
        def softmax_sample(
            rng_obj: random.Random, program_idxs: list[int], temperature: float
        ) -> int:
            if not program_idxs:
        ...
        for batch_idx in range(self.max_steps):
            ...
            outputs = run_parallel(exec_pairs)
            assert (
                len(outputs) == len(exec_pairs) == len(system_candidates) * self.bsize
            )
