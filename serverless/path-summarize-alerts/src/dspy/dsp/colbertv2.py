...
class ColBERTv2RetrieverLocal:
    def __init__(
        self, passages: list[str], colbert_config=None, load_only: bool = False
    ):
    ...
    def build_index(self):
        ...
        with Run().context(
            RunConfig(
                nranks=self.colbert_config.nranks,
                experiment=self.colbert_config.experiment,
            )
        ):
            indexer = Indexer(
                checkpoint=self.colbert_config.checkpoint, config=self.colbert_config
            )
            indexer.index(
                name=self.colbert_config.index_name,
                collection=self.passages,
                overwrite=True,
            )
    ...
    def get_index(self):
        ...
        with Run().context(RunConfig(experiment=self.colbert_config.experiment)):
            searcher = Searcher(
                index=self.colbert_config.index_name, collection=self.passages
            )
        return searcher
    ...
    def forward(self, query: str, k: int = 7, **kwargs):
        ...
        if kwargs.get("filtered_pids"):
            ...
            assert isinstance(filtered_pids, list) and all(
                isinstance(pid, int) for pid in filtered_pids
        ...
        for pid, rank, score in zip(*searcher_results, strict=False):  # noqa: B007
            results.append(
                dotdict(
                    {
                        "long_text": self.searcher.collection[pid],
                        "score": score,
                        "pid": pid,
                    }
                )
...
class ColBERTv2RerankerLocal:
    ...
    def forward(self, query: str, passages: list[str] | None = None):
        ...
        passage_score_arr = np.array(
            [score.cpu().detach().numpy().tolist() for score in tensor_scores]
        )
