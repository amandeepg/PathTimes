...
class Embeddings:
    def __init__(
        ...
        assert cache is False, (
            "Caching is not supported for embeddings-based retrievers"
        )
        ...
        self.corpus_embeddings = (
            self._normalize(self.corpus_embeddings)
            if self.normalize
            else self.corpus_embeddings
        )

        self.index = (
            self._build_faiss() if len(corpus) >= brute_force_threshold else None
        )
    ...
    def _batch_forward(self, queries: list[str]):
        ...
        pids = (
            np.tile(np.arange(len(self.corpus)), (len(queries), 1))
            if pids is None
            else pids
        )
    ...
    def _rerank_and_predict(self, q_embeds: np.ndarray, candidate_indices: np.ndarray):
        ...
        return [
            ([self.corpus[idx] for idx in indices], [idx for idx in indices])
