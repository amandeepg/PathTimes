...
class GenerateModuleInstruction(dspy.Module):
    ...
    def forward(
            ...
            for candidate_set in candidate_sets:
                for example in candidate_set:
                    if "augmented" in example.keys():
                        fields_to_use = get_signature(
                            program.predictors()[pred_i]
                        ).fields
        ...
        if self.use_task_demos:
            ...
            example_strings = gather_examples_from_sets(
        ...
        if self.program_aware:
            try:
                ...
                if self.verbose:
                    print(
                        f"Error getting program description. Running without program aware proposer. Error: {e}"
                    )
