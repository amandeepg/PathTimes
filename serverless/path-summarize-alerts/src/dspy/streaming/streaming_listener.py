...
class StreamListener:
    ...
    def _buffered_message_end_with_start_identifier(
        self, concat_message: str, start_identifier: str
    ) -> str:
        for i in range(len(concat_message)):
            if start_identifier.startswith(
                concat_message[len(concat_message) - i - 1 :]
            ):
                return True
        return False
    ...
    def receive(self, chunk: ModelResponseStream):
        adapter_name = (
            settings.adapter.__class__.__name__ if settings.adapter else "ChatAdapter"
        )
        ...
        try:
            if self._signature_field_is_citation_type():
                if chunk_citation := chunk.choices[
                    0
                ].delta.provider_specific_fields.get("citation", None):
        ...
        if (
            len(self.field_start_queue) == 0
            and not self.stream_start
            and start_indicator in chunk_message
        ):
        ...
        if len(self.field_start_queue) > 0 and not self.stream_start:
            ...
            if start_identifier in concat_message:
                ...
                value_start_index = concat_message.find(start_identifier) + len(
                    start_identifier
                )
    ...
    def _signature_field_is_citation_type(self) -> bool:
        ...
        return (
            isinstance(self.predict, Predict)
            and getattr(
                self.predict.signature.output_fields.get(
