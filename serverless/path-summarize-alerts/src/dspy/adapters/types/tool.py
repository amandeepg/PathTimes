...
class Tool(Type):
    ...
    @classmethod
    def from_mcp_tool(
        cls, session: "mcp.client.session.ClientSession", tool: "mcp.types.Tool"
    ) -> "Tool":
        """
    ...
    def __str__(self):
        desc = (
            f", whose description is <desc>{self.desc}</desc>.".replace("\n", "  ")
            if self.desc
            else "."
        )
        ...
        # Handle case where data is a dict
            ...
            if "tool_calls" in data:
                ...
                if isinstance(tool_calls_data, list):
                    return {
                        "tool_calls": [
                            cls.ToolCall(**item) if isinstance(item, dict) else item
