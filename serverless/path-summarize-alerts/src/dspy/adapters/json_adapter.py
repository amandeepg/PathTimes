...
class JSONAdapter(ChatAdapter):
    ...
    def _json_adapter_call_common(
        self, lm, lm_kwargs, signature, demos, inputs, call_fn
    ):
        ...
        has_tool_calls = any(
            field.annotation == ToolCalls for field in signature.output_fields.values()
        )
    ...
    def __call__(
        ...
        try:
            ...
            logger.warning(
                "Failed to use structured output format, falling back to JSON mode."
            )
            lm_kwargs["response_format"] = {"type": "json_object"}
            return super().__call__(lm, lm_kwargs, signature, demos, inputs)
    ...
    def format_field_structure(self, signature: type[Signature]) -> str:
        ...
        parts.append(
            format_signature_fields_for_instructions(
                signature.input_fields, role="user"
            )
        )
        parts.append("Outputs will be a JSON object with the following fields.")
        parts.append(
            format_signature_fields_for_instructions(
                signature.output_fields, role="assistant"
            )
        )
        return "\n\n".join(parts).strip()
    ...
    def enforce_required(schema_part: dict):
        ...
        if schema_part.get("type") == "object":
            ...
            if props is not None:
                ...
                schema_part["additionalProperties"] = False
        if schema_part.get("type") == "array" and isinstance(
            schema_part.get("items"), dict
        ):
