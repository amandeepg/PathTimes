    ...
    def move_type_to_front(d):
        ...
        if isinstance(d, Mapping):
            return {
                ...
                for k, v in sorted(
                    d.items(), key=lambda item: (item[0] != "type", item[0])
                )
            }
...
def get_field_description_string(fields: dict) -> str:
    ...
    for idx, (k, v) in enumerate(fields.items()):
        ...
        field_message += (
            f"\nConstraints: {v.json_schema_extra['constraints']}"
            if v.json_schema_extra.get("constraints")
            else ""
        )
...
def _format_input_list_field_value(value: list[Any]) -> str:
    ...
    return "\n".join(
        [f"[{idx + 1}] {_format_blob(txt)}" for idx, txt in enumerate(value)]
    )
