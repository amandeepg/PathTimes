    ...
    def __call__(self, *args, **kwargs) -> Prediction:
        ...
        with settings.context(caller_modules=caller_modules):
            if (
                ...
                and thread_local_overrides.get().get("usage_tracker") is None
            ):
                ...
                if isinstance(output, Prediction):
                    ...
                    and len(output) > 0
                    and isinstance(output[0], Prediction)
                ):
                    prediction_in_output = output[0]
                if not prediction_in_output:
                    raise ValueError(
                        "No prediction object found in output to call set_lm_usage on."
                    )
