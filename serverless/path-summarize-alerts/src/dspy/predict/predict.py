...
class Predict(Module, Parameter):
    ...
    def _forward_preprocess(self, **kwargs):
        ...
        assert "new_signature" not in kwargs, (
            "new_signature is no longer a valid keyword argument."
        )
    ...
    def _forward_postprocess(self, completions, signature, **kwargs):
        ...
        if (
            kwargs.pop("_trace", True)
            and settings.trace is not None
            and settings.max_trace_size > 0
        ):
    ...
    def _should_stream(self):
        ...
        if should_stream and len(stream_listeners) > 0:
            should_stream = any(
                stream_listener.predict == self for stream_listener in stream_listeners
            )
