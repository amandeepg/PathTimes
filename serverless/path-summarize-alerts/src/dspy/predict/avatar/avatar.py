    ...
    def _get_field(self, field_info: FieldInfo):
        if field_info.json_schema_extra["__dspy_field_type"] == "input":
            return dspy.InputField(
                ...
                format=field_info.json_schema_extra["format"]
                if "format" in field_info.json_schema_extra
                else None,
            )
            ...
            raise ValueError(
                f"Unknown field type: {field_info.json_schema_extra['__dspy_field_type']}"
            )
    ...
    def forward(self, **kwargs):
        ...
        while tool_name != "Finish" and (max_iters > 0 if max_iters else True):
            ...
            if tool_name != "Finish":
                ...
                args[f"action_{idx}"] = action
                args[f"result_{idx}"] = (
                    "Gathered all information needed to finish the task."
                )
                break
