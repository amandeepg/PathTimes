...
class Refine(Module):
    ...
    def forward(self, **kwargs):
        ...
        for idx, rid in enumerate(rollout_ids):
            ...
            predictor2name = {
                predictor: name for name, predictor in mod.named_predictors()
            }
            signature2name = {
                predictor.signature: name for name, predictor in mod.named_predictors()
            }
            ...
            try:
                ...
                trajectory = [
                    {"module_name": predictor2name[p], "inputs": i, "outputs": dict(o)}
                    for p, i, o in trace
                ]
                trajectory = {
                ...
                advise_kwargs = dict(
                    **modules, **trajectory, **reward, module_names=module_names
                )
                ...
                advise_kwargs = {
                    ...
                    if isinstance(v, str)
                        ...
                        recursive_mask(v), option=orjson.OPT_INDENT_2
                    ).decode()
...
def inspect_modules(program):
    ...
    for _, (name, predictor) in enumerate(program.named_predictors()):
        ...
        output.append(
            ("\n" + "\t" * 2).join(
                [""]
                + get_field_description_string(signature.output_fields).splitlines()
            )
        )
