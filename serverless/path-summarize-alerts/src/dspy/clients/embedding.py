...
class Embedder:
    ...
    def __call__(
        ...
        **kwargs: dict[str, Any],
        ...
        compute_embeddings = (
            _cached_compute_embeddings if caching else _compute_embeddings
        )
        ...
        for batch in input_batches:
            embeddings_list.extend(
                compute_embeddings(self.model, batch, caching=caching, **kwargs)
            )
    ...
    async def acall(self, inputs, batch_size=None, caching=None, **kwargs):
        ...
        embeddings_list = []
        acompute_embeddings = (
            _cached_acompute_embeddings if caching else _acompute_embeddings
        )

        for batch in input_batches:
            embeddings_list.extend(
                await acompute_embeddings(self.model, batch, caching=caching, **kwargs)
            )
        return self._postprocess(embeddings_list, is_single_input)
...
def _compute_embeddings(model, batch_inputs, caching=False, **kwargs):
    if isinstance(model, str):
        ...
        embedding_response = litellm.embedding(
            model=model, input=batch_inputs, caching=caching, **kwargs
        )
        ...
        raise ValueError(
            f"`model` in `dspy.Embedder` must be a string or a callable, but got {type(model)}."
        )
