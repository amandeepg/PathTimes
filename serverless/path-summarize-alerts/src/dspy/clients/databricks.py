            ...
            try:
                if data_format == TrainDataFormat.CHAT:
                    ...
                    client.completions.create(
                        prompt="hi", model=model_name, max_tokens=1
                    )
        ...
        try:
            with open(file_path, "rb") as f:
                target_path = os.path.join(
                    databricks_unity_catalog_path, os.path.basename(file_path)
                )
...
def _create_directory_in_databricks_unity_catalog(
    ...
    pattern = (
        r"^/Volumes/(?P<catalog>[^/]+)/(?P<schema>[^/]+)/(?P<volume>[^/]+)(/[^/]+)+$"
    )
...
def _save_data_to_local_file(
    train_data: list[dict[str, Any]], data_format: TrainDataFormat
):
    import uuid
