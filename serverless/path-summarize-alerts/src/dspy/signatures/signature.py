...
class Signature(BaseModel, metaclass=SignatureMeta):
    ...
    def with_updated_fields(
        cls, name: str, type_: type | None = None, **kwargs: dict[str, Any]
    ) -> type["Signature"]:
    ...
    def equals(cls, other) -> bool:
        ...
        for name in cls.fields.keys() | other.fields.keys():
            if name not in other.fields or name not in cls.fields:
                return False
            if (
                cls.fields[name].json_schema_extra
                != other.fields[name].json_schema_extra
            ):
                return False
        return True
    ...
    def load_state(cls, state):
        ...
        for field, saved_field in zip(
            signature_copy.fields.values(), state["fields"], strict=False
        ):
...
def ensure_signature(
    signature: str | type[Signature], instructions=None
) -> type[Signature]:
